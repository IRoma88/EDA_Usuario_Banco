...
Una entidad bancaria contrata a una empresa de marketing encargada de contactar telefónicamente a posibles clientes para determinar si están interesados o no en adquirir un deposito a plazo fijo con el banco.

La información recolectada por la empresa de marketing se encuentra en un archivo CSV con 45215 filas y 17 columnas.
...
Importamos las librerías básicas para poder limpiar los datos de la tabla y hacerla más fácil
"""
...
"""Vemos que las columnas están en inglés y que alguna columna no se sabe muy bien para qué está. Por lo que cambiaremos los nombres"""
...
"""Hay pocos datos nulos, pero muchos "unknown" en columnas que son poco relevantes para el resultado final. Las borramos. También nos aseguramos de que no se duplican los nombres como "divorced" "div." etc, que son lo mismo"""
...
"""edad                        776..... Claramente es un error, por lo que limitamos la edad a 90 años."""
...
# Imprimir el estado civil mas común entre los no_morosos
print("Estado civil con más morosos:")
print(conteo_estado_civil)
...
# Imprimir el trabajo más común entre no deudores
...
"""# Vemos en cuanto a los más destacados tanto deudores como no:
...
Con esto vemos que hay muy poca gente que no devuelve los créditos

# Buscamos el candidato ideal para conceder un crédito
"""

# Filtrar registros sin morosidad y con predicción positiva de adquirir el producto
...
# Contar ocurrencias de cada combinación de valores
conteo_total = no_morosos_que_aceptan[columnas_agrupacion].value_counts()

# Mostrar combinaciones ordenadas por frecuencia
print("📊 Combinación más común de atributos entre los que no tienen morosidad y aceptan el crédito:")
print(conteo_total.head())  # Mostramos solo las 5 más comunes (opcional)
...
"""### **El cliente más común que acepta el crédito sin morosidad es:**
1. 54 años
2. Casado
...
"""# Análisis de datos con gráficos "plt y sns"
"""
...
sns.distplot(df['edad'], bins=20, label = f'Skewness (Simetría): {df["edad"].skew()}', kde=True)
plt.legend(loc = 'best')
plt.title('edad')

sns.distplot(df[df['salario_anual'] < df['salario_anual'].quantile(0.90)]['salario_anual'], bins=30, kde=True) #Eliminamos el 1% con balance más alto para poder visualizar mejor
plt.title('Salario_anual')

df.describe()

"""Tras eliminar deudores y filtrar por edades deducimos que el perfil más llamativo es (según las gráficas):

* Edad entre 30 y 50 años y casado
* Trabaja en el sector secundario
* No dice su trabajo
* Salario por debajo de los 1500
* Sí tiene hipoteca
* No tiene otro crédito

Para próximas consultas, añadir **sexo** y **nivel educativo más claro**
